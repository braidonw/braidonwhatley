<script>
  import { scrollTo } from "lib/utils";
  import Heading from "components/type/Heading.svelte";
  import BodyText from "components/type/BodyText.svelte";
  import ButtonOutline from "components/buttons/ButtonOutline.svelte";
</script>

<div class="max-w-screen-md mt-16 sm:mt-24 md:mt-32 mx-auto">
  <Heading class="" level="1">Hey, I'm Braidon</Heading>

  <div class="mt-8">
    <BodyText>
      I manage projects that span business & transformation, technology,
      finance, machine learning & data science.
    </BodyText>
  </div>

  <div class="mt-8">
    <BodyText>
      I've also got experience developing web applications, doing machine
      learning and data science.
    </BodyText>
  </div>

  <div class="mt-8">

    <BodyText>Think we might be able to work together?</BodyText>
  </div>
  <div class="mt-8">
    <ButtonOutline on:click={() => scrollTo('#get-in-touch')}>
      Get in touch
    </ButtonOutline>
  </div>
</div>
